...
from waf.fitting.sampling import log_probability_CaHKmarginalized, log_likelihood_CaHKmarginalized, log_prior_CaHKmarginalized
...
results_file = Path('/clusterfs/dweisz/nathan_sandford/github_repos/ChemWAF/samples/EriII_FiducialMarginal.npz')
p0_file = Path('/clusterfs/dweisz/nathan_sandford/github_repos/ChemWAF/samples/EriII_FiducialMArginal_p0.npy')
...
if reload_p0 and p0_file.exists():
    ...
    while len(p0_list) < int(n_walkers / 10):
        ...
        logP = log_probability_CaHKmarginalized(
            p,
            default_par=par,
            priors=priors,
...
def log_likelihood_wrapper(p, priors, default_par, gal_par_names, floor=1e-20):
    """
    ...
    logL = log_likelihood_CaHKmarginalized(p_gal, default_par, priors['latent_FeH'], floor)
    return logL
...
def log_prior_wrapper(p, priors, gal_par_names):
    ...
    if p.ndim > 1:
        raise AttributeError('log_prior is not vectorized')
    p_gal = {par_name: p[:len(gal_par_names)][i] for i, par_name in enumerate(gal_par_names)}
    logPi = log_prior_CaHKmarginalized(p_gal, priors)
    return logPi
