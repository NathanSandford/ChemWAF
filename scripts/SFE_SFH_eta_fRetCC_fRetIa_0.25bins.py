...
while len(p0_dict) < nwalkers:
    p = dict(
        ...
        fRetCC=0.8 + 1e-1 * np.random.randn(),
        fRetIa=0.8 + 1e-1 * np.random.randn(),
    )
...
backend = emcee.backends.HDFBackend("./EriII_samples.h5", name=f"SFE_SFH_eta_fRetCC_fRetIa_0.25bins")
